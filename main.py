...
# =========================
# =========================
# SHARED HTTP CLIENT + SEMAPHORE
# =========================
# One persistent client → reuses TCP connections (no per-call overhead)
_http_client: Optional[httpx.AsyncClient] = None
# Semaphore keeps concurrent TMDB calls ≤ 5 so we never exhaust connections
...
async def lifespan(app_: FastAPI):
    global _http_client
    _http_client = httpx.AsyncClient(timeout=12, limits=httpx.Limits(max_connections=20))
    load_pickles()          # load pickles at startup
    yield
...
async def tmdb_get(path: str, params: Dict[str, Any]) -> Dict[str, Any]:
    """
    Safe TMDB GET using the shared persistent client.
    - Network errors -> 502
...
async def attach_tmdb_card_by_title(title: str) -> Optional[TMDBMovieCard]:
    ...
    Guarded by _tmdb_sem so concurrent gather calls don't exceed 5 at once.
...
# =========================
# load_pickles is now called from lifespan() above, not via on_event
def load_pickles():
...
async def search_bundle(
    ...
    # Fetch all TMDB poster cards CONCURRENTLY instead of one-by-one
    if recs:
        titles_only  = [t for t, _ in recs]
        scores_only  = [s for _, s in recs]
        cards = await asyncio.gather(
            *[attach_tmdb_card_by_title(t) for t in titles_only],
            return_exceptions=False,
        )
        for title, score, card in zip(titles_only, scores_only, cards):
